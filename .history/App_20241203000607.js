...
import * as LocalAuthentication from 'expo-local-authentication'; // Biblioteca para autenticación biométrica
import * as Location from 'expo-location'; // Biblioteca para obtener ubicación
import * as Speech from 'expo-speech'; // Biblioteca para síntesis de voz
import React, { useEffect, useState } from 'react';
import { Alert, Linking, StyleSheet, Text, TouchableOpacity, TouchableWithoutFeedback, View } from 'react-native'; // Biblioteca para abrir enlaces
...
const App = () => {
  ...
  const searchRoutes = async () => {
    const location = await Location.getCurrentPositionAsync({});
    const url = `https://www.google.com/maps/dir/?api=1&origin=${location.coords.latitude},${location.coords.longitude}&destination=Destino`;
    Linking.openURL(url);
  };

  const sendLocationViaWhatsApp = (location) => {
    const message = `Estoy en la ubicación: latitud ${location.coords.latitude}, longitud ${location.coords.longitude}`;
    const url = `whatsapp://send?text=${encodeURIComponent(message)}`;
    Linking.openURL(url).catch(() => speak("No se pudo abrir WhatsApp"));
  };
