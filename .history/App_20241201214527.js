...
const App = () => {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      Voice.destroy().then(Voice.removeAllListeners); // Limpiar el reconocimiento de voz al desmontar el componente
    };
  }, []);

  const speak = (message) => {
    console.log('Hablando:', message); // Log de mensaje hablado
  ...
  const handleVoiceCommand = (command) => {
    if (command.includes('registrar')) {
      ...
      Voice.stop(); // Detener la escucha al procesar el comando
