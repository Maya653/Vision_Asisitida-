...
const App = () => {
  ...
  const displayOptions = () => {
    ...
    startListening(); // Inicia la escucha cuando las opciones son presentadas
  };

  const startListening = async () => {
    if (isListening) {
      try {
        await Voice.start('es-MX'); // Asegúrate de iniciar la escucha en el idioma adecuado
      } catch (error) {
        console.error("Error al iniciar la escucha de voz:", error);
      }
    }
  };
