...
const App = () => {
  ...
  const handleTap = () => {
    ...
    if (tapCount === 0 || now - lastTap > 600) {
      setTapCount(1); // Primer toque
      clearTimeout(timeoutId); // Limpiar el temporizador anterior
    } else if (tapCount === 1 && now - lastTap < 600) {
      setTapCount(2); // Segundo toque
      clearTimeout(timeoutId); // Limpiar el temporizador anterior
    } else if (tapCount === 2 && now - lastTap < 600) {
      setTapCount(3); // Tercer toque
    }

    setLastTap(now);

    // Iniciar un temporizador que se ejecutará si no se detecta el tercer toque
    const newTimeoutId = setTimeout(() => {
      setTapCount(0); // Restablecer el contador si no se detecta un tercer toque
    }, 1000); // Esperar 1 segundo (ajustable según lo necesites)
  ...
  useEffect(() => {
    if (tapCount === 2) {
      ...
      setTapCount(0); // Resetear contador después del doble toque
    } else if (tapCount === 3) {
      console.log('Triple toque detectado.');
      registerFingerprint(userId); // Registrar huella
      setTapCount(0); // Resetear contador después del triple toque
    }
  }, [tapCount]);
