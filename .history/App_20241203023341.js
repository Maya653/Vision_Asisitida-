...
import * as SpeechToText from 'expo-speech-to-text'; // Para convertir voz a texto
...
const App = () => {
  ...
  const activateVoiceAssistant = () => {
    ...
    // Aquí puedes agregar un reconocimiento de voz si es necesario (ejemplo con SpeechToText)
    SpeechToText.startAsync().then((result) => {
      const userInput = result.text;
      if (userInput) {
        speak(`Has dicho: ${userInput}. Buscando rutas o activando GPS...`);
        searchRoutes(userInput); // Usar el destino proporcionado
      } else {
        speak('No pude entender lo que dijiste. Intenta de nuevo.');
      }
    }).catch((error) => {
      console.error('Error en el reconocimiento de voz:', error);
      speak('Hubo un problema al intentar reconocer tu voz.');
    });
  };
