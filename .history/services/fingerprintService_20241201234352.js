...
// Función para registrar la huella en la base de datos
const registerFingerprint = async (userId, huellaData) => {
  try {
...
const App = () => {
  ...
  const guideUser = () => {
    speak('Hola, soy tu app de visión asistida. Toca dos veces la pantalla para colocar tu huella en el sensor y autenticarte.');
  };
    ...
    if (hasHardware && supportedAuthTypes.length > 0) {
      try {
        ...
        if (result.success) {
          ...
          registerFingerprint(userId, result); // Registra la huella con un nombre aleatorio
        } else {
